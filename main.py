...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: uv run main.py --search chicken --max-prep 30 --max-calories 650",
    )
    parser.add_argument("--search", default=os.getenv("RECIPE_SEARCH", "chicken"), help="Recipe search term (default: chicken)")
    parser.add_argument("--ingredients", default=os.getenv("RECIPE_INGREDIENTS", ""), help="Comma-separated required ingredients")
    parser.add_argument("--page", type=int, default=int(os.getenv("RECIPE_PAGE", "1")), help="Positive result page (default: 1)")
    parser.add_argument("--per-page", type=int, default=int(os.getenv("RECIPE_PER_PAGE", "10")), help="Recipes requested, 1-50 (default: 10)")
    parser.add_argument("--max-prep", type=int, default=int(os.getenv("MAX_PREP_MINUTES", "30")), help="Maximum prep minutes for recommendations (default: 30)")
    parser.add_argument("--max-calories", type=int, default=int(os.getenv("MAX_CALORIES", "650")), help="Maximum calories for recommendations (default: 650)")
    parser.add_argument("--api-key", default=os.getenv("RECIPE_API_KEY", ""), help="Recipe API key; prefer RECIPE_API_KEY in .env")
    parser.add_argument("--log-level", default=os.getenv("LOG_LEVEL", "INFO"), choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], help="Logging threshold (default: INFO)")
    parser.add_argument("--log-file", default=os.getenv("LOG_FILE", "logs/weeknight-recipe-scout.log"), help="Log destination; pass an empty value to disable")
...
def fetch_json(url: str, api_key: str) -> tuple[str, dict[str, Any]]:
    ...
    request = Request(url, headers={"Authorization": f"Bearer {api_key}", "User-Agent": "weeknight-recipe-scout/1.0", "Accept": "application/json"})
...
def normalize_recipe(recipe: RecipeItem) -> dict[str, Any]:
    ...
    return {"id": recipe.id, "name": recipe.name, "description": recipe.description, "cuisine": recipe.cuisine, "difficulty": recipe.difficulty, "meal_type": recipe.meal_type, "prep_time_minutes": recipe.prep_time_minutes, "calories": recipe.calories, "ingredients": ingredients[:10], "instruction_count": len(instructions)}


def recommend_recipes(recipes: Sequence[dict[str, Any]], max_prep: int, max_calories: int) -> list[dict[str, Any]]:
    ...
    eligible = [dict(recipe) for recipe in recipes if isinstance(recipe.get("prep_time_minutes"), int) and isinstance(recipe.get("calories"), int) and recipe["prep_time_minutes"] <= max_prep and recipe["calories"] <= max_calories]
    eligible.sort(key=lambda recipe: (recipe["prep_time_minutes"], recipe["calories"], str(recipe.get("name") or "")))
    for rank, recipe in enumerate(eligible, start=1):
        recipe["recommendation_rank"] = rank
    return eligible


def summarize_payload(response: RecipeResponse, query: dict[str, Any], max_prep: int = 30, max_calories: int = 650) -> dict[str, Any]:
    ...
    return {"query": query, "recommendation_limits": {"max_prep_minutes": max_prep, "max_calories": max_calories}, "counts": {"recipes_returned": len(normalized), "recommendations": len(recommendations), "current_page": response.meta.current_page, "last_page": response.meta.last_page, "per_page": response.meta.per_page, "total": response.meta.total}, "recommendations": recommendations, "recipes": normalized, "links": {"first": response.links.first, "last": response.links.last, "prev": response.links.prev, "next": response.links.next}, "meta": {"path": response.meta.path, "language": response.meta.language}}
...
def print_summary(summary: dict[str, Any], raw_path: Path, processed_path: Path) -> None:
    ...
    if summary["recommendations"]:
        ...
        print(f"Top pick: {best['name']} ({best['prep_time_minutes']} min, {best['calories']} calories)")
    print(f"Raw response: {raw_path}")
    print(f"Processed summary: {processed_path}")


def run(api_key: str, search: str, ingredients: str, page: int, per_page: int, max_prep: int = 30, max_calories: int = 650, logger: logging.Logger | None = None) -> tuple[dict[str, Any], Path, Path]:
    """Fetch one page, validate it, save evidence, and return its summary and paths."""
    logger = logger or logging.getLogger(LOGGER_NAME)
    if not api_key:
        raise RuntimeError("RECIPE_API_KEY is missing. Put it in .env or pass --api-key.")
    query = {"search": search, "ingredients": ingredients, "page": page, "per_page": per_page}
...
def main() -> int:
    ...
    try:
        summary, raw_path, processed_path = run(args.api_key, args.search, args.ingredients, args.page, args.per_page, args.max_prep, args.max_calories, logger)
        print_summary(summary, raw_path, processed_path)
        return 0
    except HTTPError as exc:
        logger.error("Recipe API request failed with HTTP %s", exc.code)
        print(f"Error: Recipe API returned HTTP {exc.code}. Check the key, query, and service status.")
