...
def render_app() -> None:
    ...
    st.write(
        "For busy cooks who want a realistic dinner: set your time and calorie limits, then compare the best matches."
    )
    ...
    with st.sidebar:
        ...
        search = st.text_input(
            "What sounds good?", value=os.getenv("RECIPE_SEARCH", "chicken")
        )
        ...
        api_key = st.text_input(
            ...
            type="password",
            help="Stored only for this session. For local CLI use, put RECIPE_API_KEY in .env.",
        )

    if st.button("Find my best options", type="primary"):
        try:
            summary, raw_path, processed_path = run(
                api_key, search, ingredients, 1, int(per_page), max_prep, max_calories
            )
            record_run(Path(database_path), summary, processed_path)
            matches = summary["recommendations"]
            if not matches:
                st.warning(
                    "No complete recipes met both limits. Increase one limit or try a broader search."
                )
            else:
                top = matches[0]
                st.success(
                    f"Top pick: {top['name']} — {top['prep_time_minutes']} minutes and {top['calories']} calories."
                )
                columns = [
                    "recommendation_rank",
                    "name",
                ...
                st.dataframe(
                    [{key: recipe.get(key) for key in columns} for recipe in matches],
                    hide_index=True,
                    use_container_width=True,
                )
            st.caption(
                f"Evidence saved to {raw_path}; processed results saved to {processed_path}."
            )
        except Exception as exc:  # noqa: BLE001
            st.error(
                f"Could not fetch recipes: {exc}. Check the API key and internet connection, then try again."
            )
